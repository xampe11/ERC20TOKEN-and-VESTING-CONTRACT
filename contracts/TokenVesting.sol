...
contract TokenVesting is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    // Events
    event TokensLocked(address indexed beneficiary, address indexed token, uint256 amount, uint256 startTime, uint256 endTime);
     ...
     */
    function calculateClaimableAmount(address _beneficiary, uint256 _scheduleIndex) public view returns (uint256) {
     ...
     */
    function revokeVesting(address _beneficiary, uint256 _scheduleIndex) external onlyOwner whenNotPaused nonReentrant {
        if (_scheduleIndex >= vestingSchedules[_beneficiary].length) revert InvalidScheduleIndex();
     ...
     */
    function getVestingSchedule(address _beneficiary, uint256 _scheduleIndex) 
        external 
        view 
