...
async function main() {
  ...
  const START_TIME =
    (await ethers.provider.getBlock("latest")).timestamp + HALF_HOUR;
  ...
  console.log(
    "Start Time: ",
    new Date(Number(schedule.startTime) * 1000).toString()
  );

  const scheduleRelease = await tokenVesting.getReleaseSchedule(
    deployer.address,
    0
  );
